...
\begin{enumerate}
  \item \textbf{ADC-Burst-Oversampling:} Erfassung der analogen Hall-Sensorspannung über \num{32} Einzelmessungen pro Kanal und arithmetische Mittelung (Abschnitt~\ref{sec:sensor_adc}).
...
\begin{itemize}
  ...
  \item \textbf{Bedienelemente:}
    \begin{itemize}
      ...
      \item Logging-Toggle-Taster: \texttt{PB\_5} / Arduino D4 (Pull-Up)
    \end{itemize}
...
\begin{enumerate}
  ...
  \item \textbf{LED-Heartbeat-Thread} (\texttt{led\_heartbeat\_thread}, \texttt{osPriorityNormal}):
    ...
    \end{itemize}
  \item \textbf{UART-Stream-Thread} (\texttt{uart\_stream\_thread}, \texttt{osPriorityNormal}):
    \begin{itemize}
      \item Telemetrie-Ausgabe über den Hardware-UART (\texttt{PB\_10} / Arduino D6) an eine externe Arduino-Mega-Bridge.
      \item Tastergesteuert (\texttt{PB\_5} / Arduino D4): Pro Tastendruck wird genau ein Messpunkt als CSV-Zeile ausgegeben.
      \item Jede Zeile enthält: laufenden Index, burst-gemittelten Durchmesser beider Sensoren sowie den 2.~Rohwert des Bursts als Rauschvergleich.
    \end{itemize}
\end{enumerate}

\begin{lstlisting}[style=firmwarestyle, caption={Thread-Erzeugung und Initialisierung in \texttt{main()} (\texttt{main.cpp}, Z.~435--528).}, label={lst:sensor_thread_creation}]
int main() {
  led = 1;

  // UART-Banner ...
  load_calibration_from_flash(); // KVStore -> RAM
  ...
  // UART stream thread: button-triggered telemetry to Mega bridge
...
Pro Messkanal werden \num{32} aufeinanderfolgende Einzelmessungen des 12-Bit-ADC durchgeführt und arithmetisch gemittelt.
Dies realisiert einen \textbf{diskreten Tiefpass erster Ordnung} (FIR-Filter mit rechteckigem Fenster der Länge~\num{32}), der hochfrequentes Rauschen um ca.\ $10 \cdot \log_{10}(32) \approx \qty{15}{\deci\bel}$ reduziert, bevor der Wert in die Linearisierung eingeht.
Zusätzlich wird der zweite Rohwert des Bursts (Index~1) in \texttt{burst\_second\_sample[sensor\_idx]} zwischengespeichert und dem UART-Stream-Thread zur Rauschanalyse bereitgestellt.

\begin{lstlisting}[style=firmwarestyle, caption={ADC-Burst-Oversampling mit Rohwert-Capture (\texttt{read\_sensor\_raw\_adc}, Z.~157--173).}, label={lst:sensor_adc_burst}]
...
\begin{lstlisting}[style=firmwarestyle, caption={Kalibrierdatenstruktur mit sensorspezifischen Standardwerten (\texttt{main.cpp}, Z.~89--101).}, label={lst:sensor_cal_table}]
...
CalibrationPoint calibration_tables[2][3] = {
  { // Sensor 1
    {401, 1.65f}, {656, 1.75f}, {867, 1.85f}
  },
  { // Sensor 2
    {703, 1.65f}, {900, 1.75f}, {1063, 1.85f}
  }
};
\end{lstlisting}

Die kompilierten Standardwerte sind sensorspezifisch kalibriert und dienen als Fallback, wenn beim Systemstart kein gespeicherter KVStore-Datensatz gefunden wird.
Sie decken den praxisrelevanten Messbereich von \qty{1,65}{\milli\meter} bis \qty{1,85}{\milli\meter} ab.
...
\begin{center}
  \begin{tabular}{l c c c c}
    ...
    \midrule
    Sensor~1 & $\mathtt{raw}$ & 401 & 656 & 867 \\
             & $d$ [\unit{\milli\meter}] & 1{,}65 & 1{,}75 & 1{,}85 \\
    \midrule
    Sensor~2 & $\mathtt{raw}$ & 703 & 900 & 1063 \\
             & $d$ [\unit{\milli\meter}] & 1{,}65 & 1{,}75 & 1{,}85 \\
    \bottomrule
  \end{tabular}
\end{center}
...
Die Kalibrierung wird durch den Taster an \texttt{PB\_6} / Arduino D10 (Start) initiiert und durchläuft für beide Sensoren jeweils drei Referenzpunkte bei den Soll-Durchmessern \qty{1,65}{\milli\meter}, \qty{1,75}{\milli\meter} und \qty{1,85}{\milli\meter}.
...
\begin{enumerate}
  ...
  \item \textbf{Referenzpunkt-Erfassung:} Für jeden Sensor ($s \in \{1, 2\}$) und Kalibrierungspunkt ($p \in \{1, 2, 3\}$):
    \begin{enumerate}
      ...
      \item Es wird eine \textbf{tiefe zeitliche Mittelung} durchgeführt: \num{32} Burst-Ablesungen (via \texttt{read\_sensor\_raw\_adc()}) im Abstand von je \qty{5}{\milli\second} werden aufsummiert und arithmetisch gemittelt (Gesamtdauer~$\approx \qty{160}{\milli\second}$). Der resultierende ADC-Rohwert wird mit dem Soll-Durchmesser gepaart.
  ...
  \item \textbf{Persistierung:} Nach Abschluss aller sechs Messpunkte wird die gesamte Kalibrierungstabelle via \texttt{kv\_set()} persistent im internen Flash gespeichert (siehe Abschnitt~\ref{sec:sensor_cal_persistence}).
\end{enumerate}
...
\begin{lstlisting}[style=firmwarestyle, caption={Kalibrierprozedur mit tiefer zeitlicher Mittelung und Flash-Persistierung (\texttt{calibration}, Z.~204--258).}, label={lst:sensor_calibration}]
void calibration(void) {
  ...
  for (int s = 0; s < 2; s++) {
    ...
    for (int p = 0; p < 3; p++) {
      ...
      // Deep temporal averaging: 32 bursts over ~160ms
      uint32_t cal_sum = 0;
      for (int i = 0; i < 32; i++) {
  ...
  // Persist to internal flash via KVStore
  int ret = kv_set(CAL_KV_KEY, (const void*)calibration_tables,
                   sizeof(calibration_tables), 0);
  if (ret == MBED_SUCCESS) {
    printf("Calibration saved to Internal Flash!\n");
  } else {
    printf("Warning: Flash save failed (error %d)\n", ret);
  }
  printf("=== Calibration Complete ===\n\n");
}
\end{lstlisting}

\minisec{Persistente Speicherung der Kalibrierungsdaten}
\label{sec:sensor_cal_persistence}

Die Kalibrierungsdaten werden über das mbed-OS \textbf{KVStore-API} (Key-Value Store) persistent im internen Flash-Speicher des STM32F446RE abgelegt.
Das verwendete Backend ist \texttt{TDB\_INTERNAL} (TinyDB auf internem Flash), konfiguriert in \texttt{mbed\_app.json}:

\begin{lstlisting}[style=firmwarestyle, caption={KVStore-Konfiguration (\texttt{mbed\_app.json}).}, label={lst:sensor_kvstore_config}]
{"target_overrides": {
    "*": {"storage.storage_type": "TDB_INTERNAL"}
}}
\end{lstlisting}

Der KV-Schlüssel \texttt{/kv/cal\_tables} referenziert die gesamte Kalibrierungstabelle beider Sensoren als zusammenhängenden binären Block (\num{24}~Bytes: $2 \times 3 \times (\texttt{uint16\_t} + \texttt{float})$).
Beim Systemstart ruft \texttt{main()} die Funktion \texttt{load\_calibration\_from\_flash()} auf:

\begin{lstlisting}[style=firmwarestyle, caption={Laden der persistierten Kalibrierung beim Systemstart mit Dump-Ausgabe (\texttt{load\_calibration\_from\_flash}, Z.~128--155).}, label={lst:sensor_cal_load}]
void load_calibration_from_flash() {
  size_t actual_size = 0;
  int ret = kv_get(CAL_KV_KEY,
    (void*)calibration_tables,
    sizeof(calibration_tables), &actual_size);
  if (ret == MBED_SUCCESS
      && actual_size == sizeof(calibration_tables)) {
    printf("Loaded persistent calibration "
           "from internal flash (size: %u)\n",
           actual_size);
  } else {
    printf("No persistent calibration found, "
           "using standard defaults.\n");
  }

  // Dump active table in copy-pasteable C format
  printf("\n--- Active Calibration Table "
         "(copy-paste into source) ---\n");
  ...
  for (int s = 0; s < 2; s++) {
    ...
    for (int p = 0; p < 3; p++) {
      ...
      int d_frac = (int)(
        (calibration_tables[s][p].diameter_mm - d_int)
        * 100 + 0.5f);
      printf("    {%u, %d.%02df}%s\n",
        calibration_tables[s][p].raw_adc,
        d_int, d_frac, (p < 2) ? "," : "");
...
Der UART-Dump gibt die aktive Kalibrierungstabelle in syntaktisch korrekter C-Form aus, sodass gemessene Kalibrierwerte direkt als Standardwerte in den Quellcode übernommen werden können, ohne die KVStore-Abhängigkeit zu benötigen.

Ist kein gespeicherter Datensatz vorhanden (z.\,B.\ beim allerersten Programmstart oder nach einem vollständigen Re-Flash), bleiben die kompilierten Standardwerte erhalten.
...
\begin{enumerate}
  \item \textbf{Abhängigkeit vom internen Flash:} Die persistente Kalibrierung nutzt den internen Flash-Speicher des STM32F446RE über das KVStore-API (vgl.\ Abschnitt~\ref{sec:sensor_cal_persistence}). Ein vollständiges Re-Flash der Firmware löscht den KVStore-Bereich und setzt die Kalibrierdaten auf die kompilierten Standardwerte zurück.
...
\begin{enumerate}
  \item \textbf{Messwertaufnahme:} Abfrage der Sensoren mittels 32-fachem Überabtasten und stückweise linearer 3-Punkt-Skalierung.
...
Der UART-Stream-Thread ermöglicht zudem eine tastergesteuerte Telemetrieerfassung, bei der für jeden Messpunkt sowohl der burst-gemittelte als auch der zweite Rohwert des Bursts ausgegeben werden; dies erlaubt eine vergleichende Rauschanalyse im Betrieb.
